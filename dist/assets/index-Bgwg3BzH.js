(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))a(r);new MutationObserver(r=>{for(const o of r)if(o.type==="childList")for(const s of o.addedNodes)s.tagName==="LINK"&&s.rel==="modulepreload"&&a(s)}).observe(document,{childList:!0,subtree:!0});function i(r){const o={};return r.integrity&&(o.integrity=r.integrity),r.referrerPolicy&&(o.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?o.credentials="include":r.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function a(r){if(r.ep)return;r.ep=!0;const o=i(r);fetch(r.href,o)}})();const w="rdr_";let F=null;const Ie=5e3,y={save(e,t){try{localStorage.setItem(`${w}${e}`,JSON.stringify(t)),this.scheduleAutoBackup()}catch(i){console.warn("Storage save failed:",i)}},load(e,t=null){try{const i=localStorage.getItem(`${w}${e}`);return i?JSON.parse(i):t}catch(i){return console.warn("Storage load failed:",i),t}},remove(e){localStorage.removeItem(`${w}${e}`)},clearAll(){const e=[];for(let t=0;t<localStorage.length;t++){const i=localStorage.key(t);i&&i.startsWith(w)&&e.push(i)}e.forEach(t=>localStorage.removeItem(t))},getCheckboxStates(){const e={};for(let t=0;t<localStorage.length;t++){const i=localStorage.key(t);if(i&&i.startsWith(`${w}check_`)){const a=i.replace(`${w}check_`,"");e[a]=this.load(`check_${a}`,!1)}}return e},exportProgress(){const e=this.getCheckboxStates(),t=Object.values(e).filter(Boolean).length,i=Object.keys(e).length,a={version:1,appName:"The Drifter's Ledger",exportDate:new Date().toISOString(),stats:{completed:t,total:i,percentage:i>0?Math.round(t/i*100):0},preferences:{theme:this.load("theme_mode","light"),collapsedSections:this.load("collapsed_sections",[]),navCollapsedCategories:this.load("nav_collapsed_categories",[])},checkboxes:e,journal:this.load("journal","")};return JSON.stringify(a,null,2)},importProgress(e){try{const t=JSON.parse(e);return t.checkboxes&&Object.entries(t.checkboxes).forEach(([i,a])=>{this.save(`check_${i}`,a)}),t.journal&&this.save("journal",t.journal),t.preferences&&(t.preferences.collapsedSections&&this.save("collapsed_sections",t.preferences.collapsedSections),t.preferences.theme&&this.save("theme_mode",t.preferences.theme)),t.collapsedSections&&this.save("collapsed_sections",t.collapsedSections),!0}catch(t){return console.error("Import failed:",t),!1}},scheduleAutoBackup(){F&&clearTimeout(F),F=setTimeout(()=>{this.saveToLocalBackup()},Ie)},saveToLocalBackup(){const e=this.exportProgress();try{localStorage.setItem(`${w}auto_backup`,e),localStorage.setItem(`${w}last_backup`,new Date().toISOString()),console.log("📦 Auto-backup saved")}catch(t){console.warn("Auto-backup failed:",t)}},getLastBackupInfo(){return{data:this.load("auto_backup",null),timestamp:this.load("last_backup",null)}},downloadBackup(){const e=this.exportProgress(),i=`drifters-ledger-${new Date().toISOString().split("T")[0]}.json`,a=new Blob([e],{type:"application/json"}),r=URL.createObjectURL(a),o=document.createElement("a");o.href=r,o.download=i,o.click(),URL.revokeObjectURL(r),console.log(`📥 Downloaded: ${i}`)}},le="rdr-mode",de="rdr-ui-theme",he=[{id:"wood",name:"Wood Block",icon:"fa-tree"},{id:"ledger",name:"Ledger",icon:"fa-book"},{id:"wanted",name:"Wanted",icon:"fa-scroll"},{id:"saloon",name:"Saloon",icon:"fa-whiskey-glass"}];function ce(){const e=y.load(le,"day"),t=y.load(de,"wood");W(e,!1),O(t,!1)}function W(e,t=!0){document.body.setAttribute("data-mode",e);const i=document.getElementById("mode-toggle");if(i){const a=i.querySelector("i");a&&(a.className=e==="night"?"fa-solid fa-sun":"fa-solid fa-moon"),i.title=e==="night"?"Switch to Day Mode":"Switch to Night Mode"}t&&y.save(le,e)}function ue(){const e=document.body.getAttribute("data-mode")||"day";W(e==="day"?"night":"day")}function O(e,t=!0){var a;document.body.setAttribute("data-ui-theme",e),document.querySelectorAll("[data-theme-id]").forEach(r=>{r.classList.toggle("active",r.dataset.themeId===e)});const i=document.querySelector(".nav-title");i&&(i.textContent=((a=he.find(r=>r.id===e))==null?void 0:a.name)||"Ledger"),t&&y.save(de,e)}function Ne(){const e=document.body.getAttribute("data-ui-theme")||"wood";return`<div class="theme-selector">${he.map(i=>`
    <button 
  ...
  `).join("")}</div>`}function pe(){document.addEventListener("click",e=>{const t=e.target.closest("[data-theme-id]");if(t){O(t.dataset.themeId);return}e.target.closest("#mode-toggle")&&ue()})}typeof window<"u"&&(window.toggleMode=ue,window.setUITheme=O);const _e={id:"quickstart",title:"Quick Start Guide",icon:"⭐",defaultOpen:!0,items:[{id:"qs-legendary-buck",text:"Hunt Legendary Buck FIRST",tip:" West of Strawberry. The Buck Antler Trinket increases perfect pelt chances.",tip_detailed:`Now listen here, partner—before you go chasin' deer across three counties, do yourself a favor and bag that **Legendary Buck**. He roams the woods ** west of Strawberry**, a ghost of white fur between the pines.
...
*The Braithwaites lost everything. Including this.*`,reward:"1 Gold Bar ($500)",chapter:4,region:"lemoyne",verified:!0}]},$e={id:"strangers",title:"Stranger Missions",icon:"❓",defaultOpen:!1,items:[{id:"stranger-gunslingers",text:"The Noblest of Men, and a Woman",tip:"🔫 Valentine Saloon. Track 5 legendary gunslingers for Theodore Levin.",tip_detailed:`**Theodore Levin** is writing a biography on the Old West's greatest gunslingers. He needs your help tracking them down.
...
*Pearson ain't a surgeon, but the supplies help.*`,reward:"Free Medicine",verified:!0}]}]},Ue={id:"pamphlets",title:"Pamphlets & Recipes",icon:"📜",defaultOpen:!1,items:[{id:"pamph-explosive",text:"Explosive & Incendiary Ammo",tip:"💥 Most powerful ammo types. Animal Fat is key ingredient!",tip_detailed:`**Explosive and incendiary ammunition** - the most devastating rounds you can craft. Learn these recipes and you'll clear rooms in seconds.
...
*Vikings in America, centuries before Columbus.*`,reward:"Viking Hatchet + Helmet",region:"new-hanover",verified:!0},{id:"mystery-meteor",text:"Meteor Crash Sites ☄️",tip:"☄️ Roanoke Ridge. TWO fragments: collapsed shed + crater nearby!",tip_detailed:` **Location:** Roanoke Ridge, north of Roanoke Valley
...
*Space rocks don't care about property values.*`,region:"new-hanover",verified:!0,source:"screenrant.com"},{id:"mystery-aberdeen",text:"Aberdeen Pig Farm 🐷",tip:"⚠️ Scarlett Meadows. Cannibal siblings trap travelers. Money exploit!",tip_detailed:` **Location:** Scarlett Meadows, Lemoyne (NW of Bluewater Marsh)
...
*The stew smells off. Don't eat it.*`,region:"lemoyne",verified:!0,source:"gamerant.com, fandom.com"},{id:"mystery-mutant",text:"Man-Made Mutant 🧪",tip:"🏠 Abandoned house W of Van Horn. Climb to 2nd floor window!",tip_detailed:` **Location:** Abandoned house west of Van Horn Trading Post, near the water
...
*Someone was playing god here...*`,region:"new-hanover",verified:!0,source:"screenrant.com, youtube"},{id:"mystery-ikz",text:"Princess IKZ Mystery 👑",tip:"🔍 Missing princess poster in Van Horn. Chest with her initials at Fence. UNSOLVED!",tip_detailed:` **Princess Isabeau Katharina Zinsmeister** - disappeared in 1884 at age 5.
...
*The greatest unsolved mystery in RDR2...*`,region:"new-hanover",verified:!0,source:"fandom.com, reddit"},{id:"mystery-statues",text:"Strange Statues Puzzle 🗿",tip:"💰 NE of Fort Wallace. Circle of statues = gold bar puzzle!",tip_detailed:` **Location:** Northeast of Fort Wallace
...
*Someone went to a lot of trouble to hide their wealth.*`,reward:"Gold Bars",region:"new-hanover",verified:!0,source:"youtube, powerpyx.com"}]}]},Ke={id:"gang-hideouts",title:"Gang Hideouts",icon:"🏚️",defaultOpen:!1,items:[{id:"hideout-main",text:"100% Completion Hideouts",tip:"🎯 Clear all enemies. Some are story missions, some optional.",tip_detailed:`For that **100% completion**, you gotta clear **six specific gang hideouts**. Kill every last outlaw, loot the place clean, and move on to the next nest of rats.
...
*Outlaws never learn. Neither should you—keep robbin' 'em.*`,verified:!0}]}]},Je={id:"trapper",title:"Trapper Outfits",icon:"🦌",defaultOpen:!1,items:[{id:"trapper-set-bear-hunter",text:"The Bear Hunter Set",tip:"🐻 Legendary Bear Pelt required. Cold weather outfit.",tip_detailed:`**The Bear Hunter Set** - crafted from the mighty Legendary Bear.
...
*This is the single most important trinket for hunters.*`,priority:!0,verified:!0}]}]},Ye={id:"unique-hats",title:"Unique Hats & Masks",icon:"🎩",defaultOpen:!1,items:[{id:"hats-world",text:"World Location Hats",tip:"🎩 Found at specific locations. Permanently saved to wardrobe.",tip_detailed:`These **unique hats** are hidden in specific locations across the world. Pick one up and it's saved **permanently** to your wardrobe.
...
⚠️ **Bodies despawn quickly!** Loot right after killing them or lose the weapon forever.`,missable:!0,chapter:2,region:"all",reward:"4 Unique Weapons",verified:!0,source:"thegamer.com, gamerant.com"},{id:"missable-jimmy-pen",text:"Jimmy Brooks' Pen",tip:"✍️ Ch2 debt collection. SPARE him on cliff edge. He gives you his pen.",tip_detailed:`During a **Chapter 2 debt collection** mission, a witness named **Jimmy Brooks** hangs from a cliff.

✍️ **Spare his life** and he gives you his pen as thanks. It's a symbol of Arthur's honor.

⚠️ **One choice.** Drop him and the pen is gone forever.`,missable:!0,chapter:2,region:"new-hanover",reward:"Jimmy Brooks' Pen",verified:!0,source:"thegamer.com"},{id:"missable-mary-missions",text:"Mary Linton Missions (3 Parts)",tip:"💔 Complete all before Chapter 6 ends. Emotional storyline.",tip_detailed:`**Mary Linton**—Arthur's old flame. She sends letters askin' for help, and each meeting tears open old wounds.
...
💰 *Free money you'll never see if you don't pay attention.*`,missable:!0,chapter:4,region:"lemoyne",reward:"Chick's Treasure Map",verified:!0},{id:"missable-lenny",text:"A Quiet Time (Lenny Night)",tip:"🍺 Valentine bar. AUTOMATIC story mission. Not actually missable!",tip_detailed:`**"A Quiet Time"** - the legendary drunk night with **Lenny Summers** in Valentine.

🍺 This is an **automatic story mission** in Chapter 2—you CAN'T miss it!

**YNNEL! LENNYYY!**

*Listed here because it's so good it deserves mention. Enjoy every second.*`,missable:!1,chapter:2,region:"new-hanover",reward:"Legendary Memory",verified:!0,source:"fandom.com"},{id:"missable-owl-trinket",text:"Owl Feather Trinket",tip:'🦉 "Archeology for Beginners" Ch6. Help Rains Fall WITHOUT killing anyone.',tip_detailed:`In the Chapter 6 mission **"Archeology for Beginners"**, you help **Rains Fall** recover relics.

🦉 **To get the Owl Feather Trinket:**
• Agree to help Rains Fall
• Complete the mission **without killing anyone** at the camp

⚠️ If you refuse or kill soldiers, the trinket is lost forever.`,missable:!0,chapter:6,region:"ambarino",reward:"Owl Feather Trinket",verified:!0,source:"thegamer.com, gtaforums.com"},{id:"missable-guarma",text:"Guarma Animals (Chapter 5)",tip:"🦜 STUDY these in Guarma or NEVER get 100% compendium!",tip_detailed:`**Guarma** (Chapter 5) has **exclusive animals** that CANNOT be found anywhere else:
...
⚠️ **Chapter 5 is SHORT!** Study them all before you leave or your compendium is incomplete forever.`,missable:!0,chapter:5,region:"guarma",reward:"Compendium Entries",verified:!0,source:"fandom.com, thegamer.com"},{id:"missable-camp-requests",text:"Camp Companion Requests",tip:"🔍 Gang members ask for items (pipe, book, etc.). Check scouting icon.",tip_detailed:`Throughout Chapters 2-4, gang members will ask you to find specific items for them—a **harmonica**, a **pipe**, a **romance novel**, etc.
...
🗺️ *Explore everything. The world is full of secrets.*`,reward:"Compendium entries",verified:!0}]},dt={id:"companion-activities",title:"Companion Activities",icon:"🤝",defaultOpen:!1,items:[{id:"comp-ch2",text:"Chapter 2 Activities",tip:" Horseshoe Overlook camp. White icons on map.",tip_detailed:`**Horseshoe Overlook** - our first real camp after that frozen hell in Colter. Good times here, partner. The gang's still hopeful, still together.
...
🐍 *You'll wish you went with Lenny.*`,missable:!0,reward:"Coach Loot + Bounty",region:"lemoyne"},{id:"comp-charles-bank",text:"⭐ Bank Robbery with Charles",tip:"💰 SPECIAL/ULTIMATE EDITION ONLY. Rob Rhodes Bank.",tip_detailed:`**Charles** invites you to rob the **Rhodes Bank** together. This is a **Special/Ultimate Edition exclusive** activity.

💰 Big payout if you own the right edition.

⭐ *One of the few exclusive content pieces that's actually meaningful.*`,missable:!0,reward:"Bank Loot",region:"lemoyne",verified:!0,source:"fandom.com, gamerant.com"}]},{id:"comp-trophy",text:"🏆 Friends With Benefits Trophy",tip:"Complete 1 companion activity per camp (Ch2-4) for achievement.",tip_detailed:`There's an achievement called **"Friends With Benefits"** for completing at least **one companion activity in each camp** from Chapters 2-4.
...
🏆 *Easy trophy if you're paying attention. Missable forever if you're not.*`,reward:"Trophy/Achievement",verified:!0}]},ht={id:"camp-requests",title:"Camp Item Requests",icon:"🎁",defaultOpen:!1,items:[{id:"requests-jack",text:"Jack Marston's Requests",tip:'👦 Little Jack wants toys and books. Available after "A Fisher of Men".',tip_detailed:`**Jack** is just a kid—he wants simple things like toys and comic books. Bring them to him and he'll give you something in return.
...
☠️ *Silent. Deadly. Efficient.*`,missable:!0,reward:"Poison Arrows",region:"lemoyne",verified:!0},{id:"req-charles-eagle",text:"Eagle Feather for Charles (Epilogue)",tip:"🦅 Shoot eagles or find feathers. EPILOGUE only.",tip_detailed:`Shoot an **Eagle** or find loose feathers. Charles makes Horse Reviver for you.

🦅 **Epilogue only** - Visit Charles at Beecher's Hope.`,chapter:"epilogue",reward:"Horse Reviver",verified:!0,source:"gamerant.com"}]},{id:"requests-hosea",text:"Hosea Matthews' Requests",tip:"📖 Hosea wants books and medicine ingredients.",tip_detailed:`**Hosea** is a reader and a thinker. He wants specific books and medicinal plants.
...
🏆 **Reward:** Predator Bait`,missable:!0,reward:"Predator Bait",verified:!0},{id:"req-hosea-ginseng",text:"American Ginseng (×2)",tip:"🌿 Forest areas. He gives Health Cure.",tip_detailed:"Collect **2× American Ginseng** from forest areas. Hosea makes Health Cure for you.",missable:!0,reward:"Health Cure",verified:!0,source:"gamerant.com"}]},{id:"requests-pearson",text:"Pearson's Requests",tip:"👨‍🍳 Camp cook needs supplies for meals.",tip_detailed:"**Pearson** the camp cook has simple needs—help him improve camp meals.",verified:!0,subItems:[{id:"req-pearson-compass",text:"Naval Compass",tip:"⚓ Shipwrecks or naval locations. He gives Guarma Rum!",tip_detailed:`Find a **Naval Compass** at shipwrecks or naval locations. Pearson shares **Guarma Rum** and navy stories with you.

⚓ *He misses the sea more than he lets on.*`,missable:!0,reward:"Guarma Rum",verified:!0,source:"gamerant.com"},{id:"req-pearson-rabbit",text:"Rabbit Carcass",tip:"🐇 Hunt any rabbit (perfect not needed). He gives Scouting Jacket!",tip_detailed:`Bring Pearson any **Rabbit Carcass** (doesn't need to be perfect). He gives you the **Scouting Jacket** clothing item.

🧥 *A nice reward for a simple task.*`,missable:!0,reward:"Scouting Jacket",verified:!0,source:"gamerant.com"}]},{id:"requests-others",text:"Other Gang Member Requests",tip:"🎁 Various gang members have single requests. All missable after Ch4!",tip_detailed:`Every gang member has something they want. Bring it to them for rewards and relationship building.
...
⚠️ **URGENT:** Sean's time with the gang is limited. Complete this in Chapter 2!`,chapter:2,missable:!0,reward:"Fire Bottle",verified:!0},{id:"req-susan-herbs",text:"Seasoning Herbs (×2) for Susan",tip:" Wild Mint, Oregano, or Thyme. She gives Miracle Tonic.",missable:!0,reward:"Miracle Tonic",verified:!0},{id:"req-tilly-necklace",text:"Necklace for Tilly",tip:"💎 Jewelry boxes or rob wealthy NPCs. She gives Two Health Cures!",missable:!0,reward:"Two Health Cures",verified:!0,source:"gamerant.com"}]},{id:"requests-epilogue",text:"Uncle's Hangover Cure (Epilogue)",tip:"🥴 As John at Beecher's Hope. Poor Uncle needs help.",tip_detailed:`In the Epilogue, **Uncle** needs a home remedy for his perpetual hangover. Gather the ingredients.

🍺 *Uncle's condition is... chronic.*`,chapter:"epilogue",verified:!0,subItems:[{id:"req-uncle-cure",text:"Uncle's Home Remedy",tip:"📦 Milkweed, Bourbon, Peppermint, Pomade, Stringy Meat!",tip_detailed:`Gather these ingredients for Uncle's cure:
• Milkweed (riverbanks)
• Kentucky Bourbon
• Peppermint
...
🧥 **Reward:** Clothing items`,reward:"Clothing Items",verified:!0,source:"primewikis.com, gamepressure.com"}]}]},ct={id:"bounties",title:"Bounty Hunting",icon:"🤠",defaultOpen:!1,items:[{id:"bounty-valentine",text:"Valentine Sheriff (2 Bounties)",tip:'Available after "Good, Honest, Snake Oil". Alive = more $.',tip_detailed:`The **Valentine Sheriff** posts bounties on his board once you've done a little work for him. Complete the mission **"Good, Honest, Snake Oil"** in Chapter 2 and two names'll show up on that wanted board.

💰 **Pro tip**: Bringing 'em in alive always pays better. Dead's quicker, but live bounties fatten your wallet.

*These Valentine folk ain't the worst you'll hunt. Consider 'em practice.*`,chapter:2,region:"new-hanover",rewards:["$40-50 per bounty","Honor"],verified:!0,source:"rdr2.org, gamerant.com",subItems:[{id:"bounty-benedict",text:"Benedict Allbright ($40)",tip:"Rocks west of Valentine near Dakota River. MUST bring ALIVE.",tip_detailed:`**Benedict Allbright** - poison seller who's been killin' folk with tainted medicine. You'll find him perched on some **rocks west of Valentine**, near the Dakota River, contemplating his sins.

⚠️ **MUST be brought in ALIVE** - the law wants to make an example of him. If he dies, the bounty's void.

*He'll try to talk his way out. Don't let him. Hogtie and deliver.*`,reward:"$40 (Alive only)",region:"new-hanover",verified:!0},{id:"bounty-ellie",text:"Ellie Anne Swan ($50)",tip:"Camp near Wallace Station. Alive = $50, Dead = $25.",tip_detailed:`**Ellie Anne Swan** - known as "the black widow," accused of killing her husband. Found hidin' in a **camp near Wallace Station**, northwest of Valentine.

💰 **Alive = $50 | Dead = $25**

She's got a sob story ready. Up to you whether you listen or just hogtie her and ride.

*Some say she's innocent. The poster don't care.*`,rewards:["$50 Alive","$25 Dead"],region:"new-hanover",verified:!0}]},{id:"bounty-rhodes",text:"Rhodes Sheriff (3 Bounties)",tip:"Southern bounties in Lemoyne. Chapter 4+.",tip_detailed:`The **Rhodes Post Office** has three bounties on the board starting in Chapter 4 after "The Joy of Civilization" mission. Most are Lemoyne Raiders or connected criminals.

🏛️ *Gray and Braithwaite territory. Complex politics, but bounty hunting is straightforward.*`,chapter:4,region:"lemoyne",rewards:["$25-??"],verified:!0,source:"rdr2.org, gamepressure.com",subItems:[{id:"bounty-mark",text:"Mark Johnson",tip:"Bluewater Marsh, west of Kamassa River. Former robber with family. ALIVE only.",tip_detailed:`**Mark Johnson** - former stage and train robber, now hiding with his family. Found **west of Kamassa River near Bluewater Marsh**.

⚠️ **MUST bring ALIVE** - he's got family there, so approach carefully.

*Takes some tracking to locate his hideout.*`,reward:"Bring Alive",region:"lemoyne",verified:!0},{id:"bounty-robbie",text:"Robbie Laidlaw",tip:"Cabin southeast of Braithwaite Manor. Available after catching Mark Johnson. ALIVE only.",tip_detailed:`**Robbie Laidlaw** - wanted for murder. Hiding in a **cabin southeast of Braithwaite Manor**. 

⚠️ **Unlocks after catching Mark Johnson** - complete his bounty first.
⚠️ **MUST bring ALIVE**

*Dangerous and armed. Approach with Dead Eye ready.*`,reward:"Bring Alive",region:"lemoyne",verified:!0},{id:"bounty-camille",text:"Camille de Millemont",tip:"Catfish Jacksons. Lemoyne Raiders member. Dead or Alive (alive = better).",tip_detailed:`**Camille de Millemont** - a member of the Lemoyne Raiders, added with the PC release. Found at **Catfish Jacksons**.

💰 **Dead or Alive** - but bringing him in alive yields a better reward.

⚠️ *Note: This is a MALE character despite the name.*`,reward:"Dead or Alive",region:"lemoyne",verified:!0}]},{id:"bounty-strawberry",text:"Strawberry Sheriff (2 Bounties)",tip:"Western bounties in Big Valley. One in Chapter 2, one in Epilogue.",tip_detailed:`The **Strawberry Jail** has bounty posters. Joshua Brown is available early, but Bart Cavanaugh only appears in the Epilogue.

🏔️ *Mountain terrain—make sure your horse is sure-footed.*`,chapter:2,region:"west-elizabeth",rewards:["$40-??"],verified:!0,source:"rdr2.org, gamepressure.com",subItems:[{id:"bounty-joshua",text:"Joshua Brown ($40)",tip:"Old mine north of Strawberry. Shootist. ALIVE only.",tip_detailed:`**Joshua Brown** - a shootist holed up in an **old mine north of Strawberry**. The tunnels are dark and cramped.

⚠️ **MUST bring ALIVE**

💣 Patience or dynamite to smoke him out works.

💰 **$40**`,reward:"$40 (Alive only)",region:"west-elizabeth",verified:!0},{id:"bounty-bart",text:"Bart Cavanaugh",tip:"⚠️ EPILOGUE ONLY. Gang leader with crew, found passed out at camp.",tip_detailed:`**Bart Cavanaugh** - outlaw gang leader. This bounty is only available during the **Epilogue**.

You'll find him and his gang **passed out at a campsite**. Easy grab if you're quiet.

💰 Bringing him alive gets a better reward.`,reward:"Alive better",chapter:"epilogue",region:"west-elizabeth",verified:!0}]},{id:"bounty-saintdenis",text:"Saint Denis Police (2 Bounties)",tip:"Urban bounties. One early, one Epilogue only.",tip_detailed:`The **Saint Denis Police Station** has bounty posters inside. Lindsey Wofford is available after "Good, Honest, Snake Oil", while Anthony Foreman only appears in the Epilogue under certain conditions.

🏙️ *City bounties require precision. Too much chaos brings more law.*`,chapter:2,region:"lemoyne",rewards:["Varies"],verified:!0,source:"rdr2.org, gamepressure.com",subItems:[{id:"bounty-lindsey",text:"Lindsey Wofford",tip:"Fort Brennand, SW of Van Horn. Lemoyne Raiders leader. Dead or Alive.",tip_detailed:`**Lindsey Wofford** - leader of the Lemoyne Raiders. Found at **Fort Brennand, southwest of Van Horn** (NOT in Saint Denis itself).

💰 **Dead or Alive** - same payout either way, so approach how you like.

⚠️ Available after "Good, Honest, Snake Oil" mission.

*Note: Despite some confusion, this is a MALE character.*`,reward:"Dead or Alive",region:"lemoyne",verified:!0},{id:"bounty-anthony",text:"Anthony Foreman ($50)",tip:"⚠️ EPILOGUE ONLY. Doyle's Tavern. Only if NOT killed in story.",tip_detailed:`**Anthony Foreman** - you might remember him from the Trelawny missions. His bounty poster appears in the **Epilogue** after "Gainful Employment" mission.
...
💰 **$50**`,reward:"$50 (Alive only)",chapter:"epilogue",region:"lemoyne",verified:!0}]},{id:"bounty-tumbleweed",text:"Tumbleweed Sheriff (2 Bounties)",tip:"⚠️ EPILOGUE ONLY - New Austin access required.",tip_detailed:`**Tumbleweed Police Station** posts the final bounties—deep in New Austin territory. Del Lobo gang connections. Only accessible as John in the Epilogue.

🏜️ *The desert don't forgive mistakes. Bring water and bullets.*`,chapter:"epilogue",region:"new-austin",rewards:["Varies"],verified:!0,source:"rdr2.org, gamepressure.com",subItems:[{id:"bounty-joaquin",text:"Joaquin Arroyo",tip:"West of Solomon's Folly. Del Lobo gang. ALIVE only.",tip_detailed:`**Joaquin Arroyo** - Del Lobo gang member. Found **west of Solomon's Folly** in New Austin.

⚠️ **MUST bring ALIVE**

💰 Alive payout is significant. Don't shoot.`,reward:"Bring Alive",region:"new-austin",verified:!0},{id:"bounty-esteban",text:"Esteban Cortez",tip:"Mine SW of Tumbleweed. Dead or Alive.",tip_detailed:`**Esteban Cortez** - located in a **mine southwest of Tumbleweed**. Has gang backup.

💰 **Dead or Alive** - either way works.

🏆 *One of the final bounties in the game.*`,reward:"Dead or Alive",region:"new-austin",verified:!0}]}]},ut={id:"talismans",title:"Talismans & Trinkets",icon:"🔮",defaultOpen:!1,items:[{id:"talisman-group",text:"Talismans (3 ingredients, stronger effects)",tip:"🔮 Crafted at Fence. Legendary parts + jewelry + unique items.",tip_detailed:`**Talismans** are the heavy hitters—powerful charms that require **three ingredients** each: a legendary animal part, a piece of jewelry, and a unique collectible item.
...
🏆 *The best of us. Don't let anyone tell you different.*`,region:"ambarino",chapter:"epilogue",missable:!1,verified:!0}]},Tt={id:"journal",title:"XXV. Personal Journal",icon:"📓",defaultOpen:!0,isJournal:!0},v=[_e,Ge,qe,Ve,ze,Je,Ye,Ue,ut,$e,We,ct,Ke,dt,ht,je,st,Lt,...nt,lt,...Rt,Tt],Bt=[{id:"getting-started",title:"Getting Started",icon:"fa-star",sections:["quickstart"]},{id:"money-gear",title:"Money & Gear",icon:"fa-coins",sections:["treasures","weapons","satchel","camp","trapper","unique-hats","pamphlets","talismans"]},{id:"world",title:"World & Story",icon:"fa-map",sections:["strangers","legendary-animals","bounties","gang-hideouts","companion-activities","camp-requests","mysteries","missable","graves"]},{id:"challenges",title:"Challenges",icon:"fa-trophy",sections:["bandit","explorer","hunter","herbalist","sharpshooter","horseman","survivalist","gambler","weapons-expert"]},{id:"collectibles",title:"Collectibles",icon:"fa-image",sections:["collectibles","cig-gunslingers","cig-stage","cig-travel","cig-fauna","cig-horses","cig-prominent","cig-flora","cig-beauty","cig-champions","cig-vistas","cig-artists","cig-inventions"]}],Et={quickstart:"fa-play-circle",treasures:"fa-gem",weapons:"fa-gun",satchel:"fa-briefcase",camp:"fa-campground",trapper:"fa-paw","unique-hats":"fa-hat-cowboy",pamphlets:"fa-scroll",talismans:"fa-hand-sparkles",strangers:"fa-user-secret","legendary-animals":"fa-hippo",bounties:"fa-crosshairs","gang-hideouts":"fa-skull-crossbones","companion-activities":"fa-users","camp-requests":"fa-box",mysteries:"fa-magnifying-glass",missable:"fa-triangle-exclamation",graves:"fa-cross",bandit:"fa-mask",explorer:"fa-compass",hunter:"fa-bullseye",herbalist:"fa-leaf",sharpshooter:"fa-crosshairs",horseman:"fa-horse",survivalist:"fa-fire",gambler:"fa-dice","weapons-expert":"fa-gun",collectibles:"fa-box-archive","cig-gunslingers":"fa-person-rifle","cig-stage":"fa-masks-theater","cig-travel":"fa-train","cig-fauna":"fa-paw","cig-horses":"fa-horse-head","cig-prominent":"fa-user-tie","cig-flora":"fa-seedling","cig-beauty":"fa-crown","cig-champions":"fa-medal","cig-vistas":"fa-mountain-sun","cig-artists":"fa-palette","cig-inventions":"fa-gears",journal:"fa-book-open"},D="rdr_",m={checked:{},listeners:new Set},p={init(){for(let e=0;e<localStorage.length;e++){const t=localStorage.key(e);if(t&&t.startsWith(`${D}check_`)){const i=t.replace(`${D}check_`,"");try{m.checked[i]=JSON.parse(localStorage.getItem(t))||!1}catch{m.checked[i]=!1}}}console.log(`📦 State initialized: ${Object.keys(m.checked).length} items loaded`)},get(e){return m.checked[e]||!1},set(e,t){const i=m.checked[e];m.checked[e]=t;try{localStorage.setItem(`${D}check_${e}`,JSON.stringify(t))}catch(a){console.warn("Storage save failed:",a)}i!==t&&this._notifyListeners({id:e,value:t,type:"change"})},toggle(e){const t=!this.get(e);return this.set(e,t),t},subscribe(e){return m.listeners.add(e),()=>m.listeners.delete(e)},_notifyListeners(e){m.listeners.forEach(t=>{try{t(e)}catch(i){console.warn("Listener error:",i)}})},getProgress(e){let t=0,i=0;return e.forEach(a=>{a.isJournal||!a.items||a.items.forEach(r=>{t++,this.get(r.id)&&i++})}),{completed:i,total:t,percentage:t>0?Math.round(i/t*100):0}},getSectionProgress(e){if(!e.items)return{completed:0,total:0};const t=e.items.length;return{completed:e.items.filter(a=>this.get(a.id)).length,total:t}},reset(){Object.keys(m.checked).forEach(e=>{this.set(e,!1)})},getCheckedIds(){return Object.entries(m.checked).filter(([,e])=>e).map(([e])=>e)},bulkSet(e){Object.entries(e).forEach(([t,i])=>{this.set(t,i)})}},n={searchTerm:"",chapter:null,region:null,status:"all",missableOnly:!1,hasReward:!1},q=[{id:"new-hanover",name:"New Hanover"},{id:"lemoyne",name:"Lemoyne"},{id:"west-elizabeth",name:"West Elizabeth"},{id:"new-austin",name:"New Austin"},{id:"ambarino",name:"Ambarino"}],At={heartlands:"new-hanover",cumberland:"new-hanover",roanoke:"new-hanover","roanoke-ridge":"new-hanover",grizzlies:"ambarino","grizzlies-east":"ambarino","grizzlies-west":"ambarino","big-valley":"west-elizabeth","tall-trees":"west-elizabeth","great-plains":"west-elizabeth",bayou:"lemoyne","saint-denis":"lemoyne","scarlett-meadows":"lemoyne",bluewater:"lemoyne",gaptooth:"new-austin","rio-bravo":"new-austin",cholla:"new-austin","hennigans-stead":"new-austin"};function _(e){if(!e)return null;const t=e.toLowerCase();return q.some(i=>i.id===t)?t:At[t]||t}function Pt(e){var t,i,a;if(n.searchTerm){const r=n.searchTerm.toLowerCase(),o=(t=e.title)==null?void 0:t.toLowerCase().includes(r),s=(i=e.description)==null?void 0:i.toLowerCase().includes(r),l=(a=e.tags)==null?void 0:a.some(d=>d.toLowerCase().includes(r));if(!o&&!s&&!l)return!1}return!(n.chapter&&String(e.chapter||"").toLowerCase()!==n.chapter||n.region&&_(e.region)!==n.region||n.missableOnly&&!e.missable||n.hasReward&&!e.reward)}function Ht(e){return n.status==="all"?!0:n.status==="complete"?e:n.status==="incomplete"?!e:!0}function Mt(){const e=new Set,t=new Set;return v.forEach(i=>{var a;(a=i.items)==null||a.forEach(r=>{var o;if(r.chapter&&e.add(String(r.chapter).toLowerCase()),r.region){const s=_(r.region);s&&t.add(s)}(o=r.subItems)==null||o.forEach(s=>{if(s.chapter&&e.add(String(s.chapter).toLowerCase()),s.region){const l=_(s.region);l&&t.add(l)}})})}),{chapters:Array.from(e).sort((i,a)=>i==="epilogue"?1:a==="epilogue"?-1:parseInt(i)-parseInt(a)),regions:Array.from(t).sort()}}function Ot(e){n.searchTerm=e.trim()}function ee(e){n.chapter=e}function te(e){n.region=e}function Ft(e){n.status=e}function Dt(){n.missableOnly=!n.missableOnly}function It(){n.hasReward=!n.hasReward}function Nt(e){switch(e){case"search":n.searchTerm="";break;case"chapter":n.chapter=null;break;case"region":n.region=null;break;case"status":n.status="all";break;case"missable":n.missableOnly=!1;break;case"reward":n.hasReward=!1;break}}function _t(){n.searchTerm="",n.chapter=null,n.region=null,n.status="all",n.missableOnly=!1,n.hasReward=!1}function Gt(){let e=0;return n.searchTerm&&e++,n.chapter&&e++,n.region&&e++,n.status!=="all"&&e++,n.missableOnly&&e++,n.hasReward&&e++,e}function $t(){const{chapters:e,regions:t}=Mt();return`
  ...
  `}function fe(){return Bt.map(e=>{const t=e.sections.map(i=>v.find(a=>a.id===i)).filter(Boolean);return t.length===0?"":`
    ...
    `}).join("")}function zt(e){var a;if(e.isJournal)return`
  ...
  `}function Ut(){const e=document.createElement("div");e.className="sidebar-overlay",e.id="sidebar-overlay",document.body.appendChild(e);const t=document.createElement("button");t.className="nav-fab",t.id="nav-fab",t.innerHTML='<i class="fa-solid fa-bars"></i>',document.body.appendChild(t)}function jt(){document.addEventListener("click",t=>{var d;const i=t.target.closest("[data-category-toggle]");if(i){const h=i.closest(".nav-category");h.classList.toggle("expanded"),h.querySelector(".nav-category-items").classList.toggle("collapsed");const R=i.querySelector(".nav-category-arrow");R.style.transform=h.classList.contains("expanded")?"rotate(0deg)":"rotate(-90deg)";return}const a=t.target.closest(".nav-section-link");if(a){const h=a.dataset.section;ie(h),document.dispatchEvent(new CustomEvent("nav:sectionClick",{detail:{sectionId:h}})),I();return}if(t.target.closest("#filter-toggle-btn")){Kt();return}const r=t.target.closest(".filter-radio");if(r){Ft(r.dataset.status),A(".filter-radio").forEach(h=>h.classList.remove("active")),r.classList.add("active"),b();return}const o=t.target.closest(".chapter-chip");if(o){const h=o.classList.contains("active");A(".chapter-chip").forEach(g=>g.classList.remove("active")),h?ee(null):(o.classList.add("active"),ee(o.dataset.chapter)),b();return}const s=t.target.closest(".region-chip");if(s){const h=s.classList.contains("active");A(".region-chip").forEach(g=>g.classList.remove("active")),h?te(null):(s.classList.add("active"),te(s.dataset.region)),b();return}if(t.target.closest("#filter-missable")){Dt(),f("#filter-missable").classList.toggle("active"),b();return}if(t.target.closest("#filter-reward")){It(),f("#filter-reward").classList.toggle("active"),b();return}const l=t.target.closest("[data-clear]");if(l){Nt(l.dataset.clear),b();return}if(t.target.closest("#filter-reset")){_t(),b();return}if(t.target.closest("#nav-fab")){f("#nav-sidebar").classList.add("open"),(d=f("#sidebar-overlay"))==null||d.classList.add("active");return}if(t.target.closest("#sidebar-overlay")){I();return}if(t.target.closest("#nav-close")){I();return}});const e=f("#nav-search");if(e){let t;e.addEventListener("input",i=>{clearTimeout(t),t=setTimeout(()=>{Ot(i.target.value),b()},200)})}window.addEventListener("scroll",()=>{const t=A("[data-section-id]");let i=null;t.forEach(a=>{a.getBoundingClientRect().top<=150&&(i=a.dataset.sectionId)}),i&&i!==ge&&ie(i)}),p.subscribe(()=>{V(),z()})}function Kt(){M=!M;const e=f("#filter-panel"),t=f("#filter-toggle-btn");e&&e.classList.toggle("open",M),t&&t.classList.toggle("active",M)}function ie(e){ge=e,A(".nav-section-link").forEach(i=>{i.classList.toggle("active",i.dataset.section===e)});const t=f(`.nav-section-link[data-section="${e}"]`);if(t){const i=t.closest(".nav-category");i&&!i.classList.contains("expanded")&&(i.classList.add("expanded"),i.querySelector(".nav-category-items").classList.remove("collapsed"))}}function I(){var e;(e=f("#nav-sidebar"))==null||e.classList.remove("open")}function b(){console.log("🔍 applyFilters called, activeFilters:",n),Jt(),Yt(),window.dispatchEvent(new CustomEvent("filters-changed",{detail:{...n}}))}function Jt(){const e=Gt(),t=f("#filter-count");t&&(t.textContent=e,t.style.display=e>0?"flex":"none")}function Yt(){const e=f("#active-filters");e&&(e.innerHTML=Wt())}function V(){v.forEach(e=>{if(e.isJournal||!e.items)return;const t=f(`[data-count-section="${e.id}"]`);if(!t)return;const i=e.items.filter(s=>p.get(s.id)).length,a=e.items.length,r=a>0?i/a*100:0;t.textContent=`${i}/${a}`,t.classList.remove("progress-none","progress-low","progress-mid","progress-full"),r===0?t.classList.add("progress-none"):r<30?t.classList.add("progress-low"):r<60?t.classList.add("progress-mid"):r<100?t.classList.add("progress-high"):t.classList.add("progress-full");const o=t.closest(".nav-section-link");o&&o.classList.toggle("section-complete",i===a&&a>0)})}function z(){let e=0,t=0;v.forEach(o=>{o.isJournal||!o.items||(e+=o.items.length,t+=o.items.filter(s=>p.get(s.id)).length)});const i=e>0?Math.round(t/e*100):0,a=f("#nav-stats-fill"),r=f("#nav-stats-text");a&&(a.style.width=`${i}%`),r&&(r.textContent=`${t} / ${e} (${i}%)`)}function Qt(){const e=f("#nav-categories");e&&(e.innerHTML=fe()),V(),z()}const Xt={treasures:{color:"#eab308",name:"gold"},explorer:{color:"#eab308",name:"gold"},collectibles:{color:"#eab308",name:"gold"},quickstart:{color:"#22c55e",name:"green"},satchel:{color:"#22c55e",name:"green"},camp:{color:"#22c55e",name:"green"},missable:{color:"#dc2626",name:"red"},"legendary-animals":{color:"#dc2626",name:"red"},hunter:{color:"#dc2626",name:"red"},bounties:{color:"#f97316",name:"orange"},bandit:{color:"#f59e0b",name:"amber"},gambler:{color:"#f59e0b",name:"amber"},herbalist:{color:"#f59e0b",name:"amber"},horseman:{color:"#f59e0b",name:"amber"},sharpshooter:{color:"#f59e0b",name:"amber"},survivalist:{color:"#f59e0b",name:"amber"},"weapons-expert":{color:"#f59e0b",name:"amber"},strangers:{color:"#3b82f6",name:"blue"},"companion-activities":{color:"#3b82f6",name:"blue"},"camp-requests":{color:"#3b82f6",name:"blue"},graves:{color:"#3b82f6",name:"blue"},mysteries:{color:"#3b82f6",name:"blue"}},Zt={color:"#9ca3af",name:"gray"};function U(e){return e.startsWith("cig-")?{color:"#22c55e",name:"green"}:Xt[e]||Zt}function ei(e,t){var d,h;const i=p.get(e.id),a=((d=e.subItems)==null?void 0:d.length)>0,r=((h=e.tip_detailed)==null?void 0:h.length)>0,{color:o,name:s}=U(t);let l={completed:0,total:0,percent:0};return a&&(l.total=e.subItems.length,l.completed=e.subItems.filter(g=>p.get(g.id)).length,l.percent=Math.round(l.completed/l.total*100)),`
  ...
  `}function ai(e,t){const{color:i}=U(t);return`
    ...
    <div class="settings-panel" id="settings-panel-content">
      ...
      <div class="settings-body">
        ...
        <div class="settings-section">
          ...
          <label class="settings-switch">
            <input type="checkbox" id="video-toggle" ${E?"checked":""}>
            <span class="switch-slider"></span>
          </label>
        </div>
      </div>
  ...
  `}function Li(e){e.querySelector("#settings-fab").addEventListener("click",()=>{Ti()}),e.querySelector("#settings-close").addEventListener("click",()=>{N()}),e.querySelectorAll(".mode-btn").forEach(t=>{t.addEventListener("click",()=>{const i=t.dataset.mode;Bi(i),e.querySelectorAll(".mode-btn").forEach(a=>a.classList.remove("active")),t.classList.add("active")})}),e.querySelectorAll(".theme-option").forEach(t=>{t.addEventListener("click",()=>{const i=t.dataset.theme;Ei(i),e.querySelectorAll(".theme-option").forEach(a=>a.classList.remove("active")),t.classList.add("active")})}),e.querySelector("#audio-toggle").addEventListener("change",t=>{B=t.target.checked,localStorage.setItem(k.audio,B);const i=B?.3:0;be(i),ye(i)}),e.querySelector("#video-toggle").addEventListener("change",t=>{E=t.target.checked,localStorage.setItem(k.video,E);const i=document.querySelector(".video-background");i&&(i.style.display=E?"block":"none")}),document.addEventListener("click",t=>{T&&!e.contains(t.target)&&N()}),document.addEventListener("keydown",t=>{t.key==="Escape"&&T&&N()})}function Ti(){const e=document.getElementById("settings-panel-content");T=!T,e.classList.toggle("open",T)}function N(){const e=document.getElementById("settings-panel-content");T=!1,e.classList.remove("open")}function Bi(e){H=e,document.body.dataset.mode=e,localStorage.setItem(k.mode,e),W(e),document.dispatchEvent(new CustomEvent("settings:modeChange",{detail:{mode:e}}))}function Ei(e){C=e,document.body.dataset.uiTheme=e,localStorage.setItem(k.theme,e),O(e),document.dispatchEvent(new CustomEvent("settings:themeChange",{detail:{theme:e}}))}const u=e=>document.querySelector(e),Ai=30;function Pi(){const e=u("#particles");if(e)for(let t=0;t<Ai;t++){const i=document.createElement("div");i.className="particle",i.style.top=`${Math.random()*100}vh`,i.style.left=`${Math.random()*100}vw`;const a=6+Math.random()*8,r=Math.random()*6;i.style.animation=`dust ${a}s linear infinite`,i.style.animationDelay=`-${r}s`;const o=2+Math.random()*2;i.style.width=`${o}px`,i.style.height=`${o}px`,i.style.opacity=(.3+Math.random()*.4).toString(),e.appendChild(i)}}const Re="journal";function Hi(){const e=document.createElement("button");e.className="journal-fab",e.id="journal-fab",e.title="Personal Journal",e.innerHTML='<i class="fa-solid fa-book"></i>',document.body.appendChild(e);const t=document.createElement("div");t.className="journal-modal",t.id="journal-modal",t.innerHTML=Mi(),document.body.appendChild(t),Oi(),Ii()}function Mi(){return`
  ...
  `}function Oi(){var t;(t=u("#journal-fab"))==null||t.addEventListener("click",Fi),document.addEventListener("click",i=>{i.target.closest("[data-close-modal]")&&oe()}),document.addEventListener("keydown",i=>{i.key==="Escape"&&Di()&&oe()});const e=u("#journal-textarea");if(e){let i;e.addEventListener("input",()=>{Le(),ne("Saving..."),clearTimeout(i),i=setTimeout(()=>{y.save(Re,e.value),ne("Saved")},500)})}}function Fi(){const e=u("#journal-modal");e&&(e.classList.add("open"),document.body.style.overflow="hidden",setTimeout(()=>{var t;(t=u("#journal-textarea"))==null||t.focus()},100))}function oe(){const e=u("#journal-modal");e&&(e.classList.remove("open"),document.body.style.overflow="")}function Di(){var e;return(e=u("#journal-modal"))==null?void 0:e.classList.contains("open")}function Ii(){const e=y.load(Re,""),t=u("#journal-textarea");t&&(t.value=e,Le())}function Le(){const e=u("#journal-textarea"),t=u("#journal-char-count");if(e&&t){const i=e.value.length;t.textContent=`${i} character${i!==1?"s":""}`}}function ne(e){const t=u("#journal-save-status");t&&(t.textContent=e,t.classList.toggle("saving",e==="Saving..."))}function Ni(){const{completed:e,total:t,percent:i}=Te();return`
  ...
  `,(t=u("#btn-burn"))==null||t.addEventListener("click",$i),(i=u("#btn-export"))==null||i.addEventListener("click",Wi),(a=u("#btn-import"))==null||a.addEventListener("click",()=>{var o;return(o=u("#import-file"))==null?void 0:o.click()}),(r=u("#import-file"))==null||r.addEventListener("change",qi))}function $i(){confirm(`🔥 Burn the history?

This will reset ALL checkboxes and clear your journal.

This action cannot be undone.`)&&(y.clearAll(),location.reload())}function Wi(){const e=y.exportProgress(),t=new Blob([e],{type:"application/json"}),i=URL.createObjectURL(t),a=document.createElement("a");a.href=i,a.download=`drifters-ledger-${new Date().toISOString().split("T")[0]}.json`,a.click(),URL.revokeObjectURL(i)}function qi(e){var a;const t=(a=e.target.files)==null?void 0:a[0];if(!t)return;const i=new FileReader;i.onload=r=>{var s;const o=(s=r.target)==null?void 0:s.result;typeof o=="string"&&(y.importProgress(o)?(alert("✅ Progress imported successfully! Reloading..."),location.reload()):alert("❌ Failed to import progress. Invalid file format."))},i.readAsText(t),e.target.value=""}async function se(){console.log("🤠 Initializing The Drifter's Ledger V2..."),ce(),pe(),p.init(),Pi(),document.getElementById("header")&&_i(),document.getElementById("sidebar")&&qt(),ci("checklist"),Hi(),document.getElementById("actions")&&Gi(),Si("settings-panel"),Vi(),$(),console.log("✅ V2 initialized successfully!")}function Vi(){document.addEventListener("nav:sectionClick",e=>{const{sectionId:t}=e.detail;bi(t)}),document.addEventListener("checklist:change",()=>{Qt(),$()}),document.addEventListener("settings:themeChange",()=>{}),document.addEventListener("settings:modeChange",()=>{}),p.subscribe(()=>{$()})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",se):se();
